...
public class SpiderRun {
	...
	/**
	 * Update queue indicator. Use <code>refreshQueue</code> method to update
	 * indicator.
	 * 
	...
	/**
	 * Update site map tree. Use <code>updateData</code> method to update tree.
	 * 
				...
				for (Map.Entry<String, Boolean> mapping : urlMap.entrySet()) {
					...
					if (!mapping.getValue()) {
						...
						try {
							...
							/*
							 * Throws exception if the URL isn't support to
							 * parse. Considerate it as the end of the current
							 * index.
							 */
							...
							for (Element link : imports) {
								...
								if (!isHostOnly || link.attr("abs:href").contains(option.getHost())) {
									...
									if (!newUrl.equals("") && !urlMap.containsKey(newUrl)
											&& !newURLMap.containsKey(newUrl)) {
		...
		try {
			...
			for (Map.Entry<String, Boolean> mapping : urlMap.entrySet()) {
				...
				if (!result.getAccessedURLs().containsKey(mapping.getKey())) {
					...
					try {
						// Set connection URL to get response headers, ignore it
						// when got IOException
						...
						if (connection.Validate()) {
							...
							try {
								...
								/*
								 * Throws exception if the URL isn't support to
								 * parse. This kind of page whose data is only
							...
							// Get the path array prepared to insert into the
							// nodes
